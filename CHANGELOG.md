...
本补丁收口 v1.9.0 发布后发现的 Cloud 嵌入协作问题，不扩展结构分析能力或数值范围。

重点修正：

- 首轮嵌入握手在宿主已加载时仍可可靠抵达，避免 Cloud 已准备而 Solver 未收到握手。
- 宿主主题只在受信握手后同步，Solver 仍保留用户主动切换主题与系统设置的归属。
- Cloud 协商的文件菜单继续承载新建、打开、保存、另存、工程、版本和分享；已保存、保存中、只读与待保存状态使用清晰且可访问的视觉反馈。
- 修复嵌入工作台右侧参数面板在聚焦输入时的内容裁切，不改动任何结构模型或计算结果。

兼容边界：

- Host Protocol 仍为 `1.0.0`；所有新行为均是已有协议上的兼容性修正，不引入账号、云存储、数据库迁移或新的求解域。
- 不修改梁系、平面桁架、平面框架、GNA/GNIA、线性屈曲、项目文件或导出数值。
- 候选、镜像、线上部署、annotated Tag 与公网验收分别记录在 [v1.9.1 发布验收](docs/verification/release-1-9-1-acceptance.md)；未完成项不提前表述为已发布。
