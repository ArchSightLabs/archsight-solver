...
test("buildTrussMemberLengthLegendRows splits long equal-length member groups into bounded rows", () => {
  const dimensions = Array.from({ length: 8 }, (_, index) =>
    buildTrussMemberLengthDimension(`M${index + 1}`, { x: 0, y: 0 }, { x: 3, y: 0 }, 3),
  ).filter((dimension): dimension is NonNullable<typeof dimension> => Boolean(dimension));

  assert.deepEqual(buildTrussMemberLengthLegendRows(dimensions, 220, 12), ["M1=M2=M3=M4=3 m", "M5=M6=M7=M8=3 m"]);
});

test("buildTrussMemberLengthLegendRows further splits equal-length groups that exceed display width", () => {
  const dimensions = Array.from({ length: 6 }, (_, index) =>
    buildTrussMemberLengthDimension(`M${index + 1}`, { x: 0, y: 0 }, { x: 3, y: 0 }, 3),
  ).filter((dimension): dimension is NonNullable<typeof dimension> => Boolean(dimension));

  assert.deepEqual(buildTrussMemberLengthLegendRows(dimensions, 260, 12), ["M1=M2=M3=M4=3 m", "M5=M6=3 m"]);
  assert.deepEqual(buildTrussMemberLengthLegendRows(dimensions, 120, 12), ["M1=M2=M3=M4=3 m", "M5=M6=3 m"]);
  assert.deepEqual(buildTrussMemberLengthLegendRows(dimensions, 70, 12), ["M1=M2=3 m", "M3=M4=3 m", "M5=M6=3 m"]);
});
