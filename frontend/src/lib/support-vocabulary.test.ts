...
test("框架支座概览显示滚动支座角度、弹性约束和实际释放自由度", () => {
  ...
  assert.equal(
    ...
    "弹性边界节点 · uy 弹性约束 5000 kN/m，释放 ux、rz",
  );
...
test("框架节点弹性约束入口使用共享短文案", () => {
  ...
  assert.equal(frameSpringBoundaryTitle(), "弹性边界");
  assert.match(frameSpringBoundaryHint(), /ux、uy 或 rz/u);
  assert.match(frameSpringBoundaryHint(), /有限刚度/u);
  assert.match(frameSpringBoundaryHint(), /0 刚度不计入支座节点/u);
  assert.equal(frameSpringBoundaryEmptyHint(), "未设置弹性边界；仅按支座类型形成刚性边界。");
  assert.equal(frameSpringBoundaryAddLabel(), "添加边界");
  assert.doesNotMatch(
    [frameSpringBoundaryTitle(), frameSpringBoundaryHint(), frameSpringBoundaryEmptyHint(), frameSpringBoundaryAddLabel()].join("\n"),
    /弹簧|自由度弹性约束/u,
  );
});
