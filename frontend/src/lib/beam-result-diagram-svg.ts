  ...
  ${
    viewSettings?.showExtremeLabel !== false
      ...
      <g data-keypoint-kind="${escapeSvg(point.kind)}" aria-label="${escapeSvg(`${kindLabel} ${stationLabel} ${valueLabel}`)}">
        ${viewSettings?.showKeyPointTypes ? `<text x="${n(layout.textX)}" y="${n(layout.valueY - (compact ? 12 : 14))}" fill="${COLORS.label}" stroke="${COLORS.textHalo}" stroke-width="${STATION_TEXT_HALO_WIDTH}" paint-order="stroke" font-size="${compact ? 9 : 11}" font-family="${DIAGRAM_NUMERIC_FONT}" font-weight="600">${escapeSvg(kindLabel)}</text>` : ""}
