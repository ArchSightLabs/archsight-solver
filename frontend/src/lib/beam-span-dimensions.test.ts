...
test("buildBeamSpanDimensionLegendRows groups equal span lengths and separates differing lengths", () => {
  ...
  assert.deepEqual(buildBeamSpanDimensionLegendRows(segments, 100), ["(1)=(2)=4m", "(3)=3m"]);
  assert.deepEqual(buildBeamSpanDimensionLegendRows(segments, 420), ["(1)=(2)=4m", "(3)=3m"]);
});
...
test("buildBeamSpanDimensionLegendRows groups non-adjacent equal span lengths", () => {
  ...
  assert.deepEqual(buildBeamSpanDimensionLegendRows(segments, 420), ["(1)=(3)=4m", "(2)=3m"]);
});
