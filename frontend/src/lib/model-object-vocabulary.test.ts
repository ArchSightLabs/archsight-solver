...
test("模型对象词表区分梁系、框架和桁架对象语义", () => {
  ...
  assert.equal(modelObjectVocabulary("beam").navigatorTitle, "跨段、支座与荷载");
  assert.equal(modelObjectVocabulary("beam").memberGroupLabel, "跨段");
  assert.equal(modelObjectVocabulary("beam").supportGroupLabel, "支座");
...
test("模型画布摘要指标来自模型对象词表", () => {
  ...
  assert.deepEqual(modelObjectMetricRows(workspace, "beam").map((row) => row.label), ["跨段", "总长度", "支座"]);
...
test("模型对象词表提供统一的计数短语和作用对象荷载标签", () => {
  ...
  assert.equal(modelObjectCountPhrase("beam", "member", 2), "2 跨");
  assert.equal(modelObjectCountPhrase("beam", "support", 3), "3 个支座");
...
test("对象编辑器成员术语从共享模型对象词表派生", () => {
  assert.equal(modelObjectMemberTerm("beam"), "跨段");
