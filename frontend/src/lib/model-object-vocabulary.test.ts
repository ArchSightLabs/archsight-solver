...
test("模型画布摘要指标来自模型对象词表", () => {
  ...
  assert.deepEqual(modelObjectMetricRows(workspace, "frame").map((row) => row.label), ["节点", "构件", "荷载"]);
  assert.deepEqual(modelObjectMetricRows(workspace, "truss").map((row) => row.label), ["节点", "杆件", "荷载"]);
});
