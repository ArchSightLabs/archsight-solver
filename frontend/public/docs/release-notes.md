...
本版把公开验证案例进一步变成“可验证的结构力学学习与复核工作台”：先做结构判断，再计算、看图、核对解析证据并导出复核记录；项目继续保持 Apache-2.0 开源、免费和二维线弹性静力分析边界。

重点变化：

- 新增梁、平面桁架、平面框架三条五分钟学习路径，每条都围绕三项枚举化预判、实际求解、图形核对和 A 级教材解析证据展开。
- DOCX、XLSX 与可信计算包记录路径、选择、标准结论和一致性状态；展示文案由 benchmark 事实源解析，不接受客户端伪造标签。
- 普通工程保持原有工作台，不新增账号、课程、班级、作业或订阅；匿名里程碑仍不采集模型、参数、结果、文件、自由文本或身份。

发布验证：

- 后端、独立刚度法、前端、三浏览器学习路径、真实 DOCX/XLSX、发行包和 Docker 候选均纳入 v1.8.0 发布门禁。

## v1.7.0

发布时间：2026-08-09

本版聚焦可信计算与开发者交付：计算结果可以独立携带、校验和复算，项目继续保持 Apache-2.0 开源、免费。
...
- 后端、独立刚度法、前端、三浏览器导出与复算、发行包隔离安装及 Docker 镜像验收全部通过。
