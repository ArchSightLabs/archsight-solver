...
除非你在提交时明确标注 “Not a Contribution” 或项目维护者另行书面接受其他授权条款，向本仓库提交的 Pull Request、patch、issue 附件、benchmark 算例、文档和测试样例，默认表示你同意将该贡献按 Apache-2.0 许可纳入本项目。

提交贡献前，请确认你有权公开该内容，且内容不包含客户资料、商业软件源码、第三方内部资料、专有规则库、受保密义务约束的数据或未经授权的品牌素材。
...
如果文件格式不适合写入头部，请确保同目录或仓库级许可证信息能够清晰覆盖该文件。
...
CI 还会在 `windows-latest` 上从锁文件重新安装可选依赖、加载 Rolldown 的 Windows 原生绑定并执行生产构建，防止仅在 Linux 上通过而 Windows 首次克隆无法构建。
