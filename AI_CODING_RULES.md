...
- “模板”表示将参数模板写入当前模型，不得命名为“案例”；“对象”统一承载当前结构体系的高频工程对象与属性编辑：梁系以跨段、支座、荷载为主，平面框架和平面桁架以节点、构件/杆件、支座、荷载为主；“文本”统一承载全量文本模型导入/导出；“基本”用于模型口径、单位、主要结果与校核方法简介。
- 界面一致性只约束一级信息架构、视觉密度和交互质量，不等于三类分析对象必须使用同一种建模心智。梁系主路径必须保持结构工程常用的 **跨段 / 支座 / 荷载** 口径；内部可以映射为支座节点与梁单元，但主操作、按钮、属性标题和空状态不得把“节点 / 杆件对象树”作为梁系第一心智。
- 梁系对象页若保留在统一五页签中，应优先展示“跨段、支座、荷载”和跨段刚度/材料维护；“支座节点、杆件、v / θz”等词只用于计算假定、文本模型、报告或高级说明，不应替代高频入口的“跨段 / 支座”。
- 任何为了框架/桁架对象统一而改动梁系界面时，必须显式检查梁系是否比行业常见连续梁/梁系软件更复杂；若复杂度上升，应退回梁系专用表达或做高级折叠，不得以统一为理由牺牲梁系易用性。
...
### 6.3 模型权限分级与求解核心保护 (Model Permission Levels)

本项目允许多智能体协作，但任务分派必须按“错误代价”分级，而不是按表面复杂度或模型偏好分派。所有智能体在接手任务前，必须先判断任务风险等级；触及更高等级时，必须提醒用户并升级主控复核。

- **Level 0：只读分析**。任何模型均可执行；仅允许阅读、搜索、解释、归纳、提出风险和建议，不得修改代码、文档或配置。
- **Level 1：低风险修改**。Gemini、Antigravity 等辅助智能体可以主导；适用于文档、错误信息、测试补充、UI 文案、非核心脚本、示例算例、benchmark harness、结果可视化等错了容易发现且容易回滚的改动。
- **Level 2：中风险修改**。Gemini 等辅助智能体可以提出方案、生成草稿或做局部辅助，但必须由 Codex 或具备同等代码审查能力的主控智能体复核后执行；适用于接口适配、模块拆分、局部重构、导出链路、缓存链路、任务调度、跨前后端数据流等改动。
- **Level 3：高风险核心修改**。只能由 Codex 或具备同等能力的主控智能体主导；Gemini 等辅助智能体只能做只读分析、benchmark 补充、失败样例归纳或测试建议。范围包括单元刚度矩阵、自由度组织、边界条件、约束方程、矩阵装配、稀疏求解接口、数值稳定性、收敛判断、几何内核、规则引擎核心、架构边界和核心数据结构。
- 凡触及 Level 3，必须先显式提醒用户该任务属于“求解核心保护边界”，并在实施前明确可复现算例、数值容差、回归测试和与既有公开 benchmark 的一致性检查；不得只凭单一演示样例或 UI 结果判断正确。
- 对 Level 2 / Level 3 任务，推荐工作流为：Codex 确定架构方案和修改边界 -> 用户确认边界 -> Codex 完成核心 diff 或给出极细执行说明 -> Gemini / Antigravity 执行低风险辅助项（运行、整理、补测试、文档）-> Codex 最终审查。

### 6.4 专业表达与指标命名 (Professional Naming Rules)
