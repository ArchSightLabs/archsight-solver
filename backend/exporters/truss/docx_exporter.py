...
def export_docx(
    ...
    return ExportArtifact(
        buffer=output,
        filename=export_filename(solution["projectName"], "truss", "docx"),
...
def _add_member_figures(doc, solution: Dict[str, Any], report_images: Optional[Dict[str, str]], options: Dict[str, str]) -> None:
    ...
    if include_overlay_figures(options) or include_traditional_figures(options):
        for figure in report_figures_for_scope(TRUSS_REPORT_OVERLAY_FIGURES, include_all):
            ...
            if not image:
                missing_labels.append(figure.title)
                continue
...
def _add_preview_figure(doc, report_images: Optional[Dict[str, str]]) -> None:
    ...
    if image:
        add_png_figure(
            doc,
            image,
            "图 2-1 平面桁架受力变形示意（节点、杆件编号、尺寸与荷载标注同图显示；蓝色为放大后的变形线）",
        )
        return

    add_report_note(
        doc,
        "说明：未收到前端同源受力变形图，已跳过受力变形插图；请从工作台导出 DOCX 以生成带节点编号、杆件编号、尺寸与荷载标注、变形线的预览图。",
    )
