...
def _build_failure_review_model_from_job(job_id: str, data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if status != "failed":
        raise ApiError(f"指定作业未失败，无法生成失败审查材料，状态：{status or 'unknown'}", code="COMMON_JOB_NOT_READY", status_code=400)

    forbidden_fields = [field for field in ("inputId", "completedStages", "stableErrorCode", "objectRefs", "diagnostics", "hashes", "suggestedActions") if field in data]
    if forbidden_fields:
