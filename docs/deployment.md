...
```powershell
docker build -t archsight-solver:v1.6.1 -t registry.example.com/example-namespace/archsight-solver:v1.6.1 .
```

推送：

```powershell
docker push registry.example.com/example-namespace/archsight-solver:v1.6.1
```
...
推送 `v1.6.1` 形式的 Git tag 后，GitHub Actions 发布工作流会复跑版本、后端、前端、Playwright 和 Docker 门禁，并生成以下可追踪制品：

- `ghcr.io/<owner>/archsight-solver:v1.6.1` 不可变版本镜像。
- Docker 镜像归档 `archsight-solver-v1.6.1.tar.gz`。
