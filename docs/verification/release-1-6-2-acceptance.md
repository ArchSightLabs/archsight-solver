...
| 构建后镜像 | 以运行时 allowlist 启动发布镜像，Reference Host 连接容器 URL，复跑嵌入模式 canonical 主链路 |
...
Docker 发布镜像应以精确宿主白名单启动，并以 `ARCHSIGHT_SOLVER_E2E_URL` 指向容器复跑 v1.6.2 嵌入验收。Tag Release 还必须生成 Trivy JSON 报告与 SPDX JSON SBOM；存在已有修复版本的 HIGH / CRITICAL 漏洞时不得发布。
...
本节记录正式发布提交前已经取得的候选证据。推送 `v1.6.2` tag 后，Release 工作流必须在相同提交上复跑版本、测试、镜像、漏洞扫描和制品门禁；目标镜像仓库推送与线上更新不由本验收结果代替。
