...
| 后端测试 | 548 passed，2 skipped |
| 前端 lint / unit / build / production audit | lint 通过；395 passed；生产构建通过；0 vulnerabilities |
| 三浏览器与跨版本 Playwright | canonical 独立/嵌入在 Chromium、Firefox、WebKit 共 6 项通过；WebKit 当前版本生命周期/诊断/结果有效性 13 项通过；v1.6.1/v1.6/v1.5 Chromium 回归 19 项通过 |
| 构建后 Docker 健康与 Host canonical | `archsight-solver:1.6.2-rc` 构建成功；最终镜像 `sha256:fa4368d9…`；非 root `app`；health `healthy`；运行时 CSP 包含精确 Host origin；容器嵌入 canonical 1 项通过 |
| Trivy HIGH / CRITICAL | Trivy 0.69.3 扫描 Debian 13.6 与 Python 依赖；`--ignore-unfixed` 下 HIGH 0、CRITICAL 0 |
| SPDX SBOM | Syft 生成 SPDX-2.3，识别 120 个 packages，结构校验通过 |
