...
本文记录 v1.5.0 发布前必须复核的建模易用性、荷载场景、结果来源和导出证据链。v1.5.0 是高可用建模工作台大版本，不新增第四类分析域，但必须证明三类既有模块能更快完成首个可计算模型，并且 `loadCases` / `loadCombinations` 编辑、结果切换和导出来源一致。
...
npm --prefix frontend run test:visual -- release-1-5-quick-modeling.spec.ts --project=chromium
