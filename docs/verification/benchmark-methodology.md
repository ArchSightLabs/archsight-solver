...
| 层级 | 来源 | 用途 |
|---|---|---|
| 教材解析解 | 简支梁、悬臂梁、静定桁架等可手算工况 | 校验基本公式、单位换算、边界条件和符号约定 |
| 独立刚度法基线 | 独立实现或可复核矩阵法算例 | 校验框架、桁架和连续梁的刚度装配行为 |
| 内部回归基线 | 高频模板、荷载转换和字段契约回归 | 防止后续迭代引入结果漂移 |

内部回归基线不能包装成外部独立验证。文档和计算书应区分“解析解”“独立基线”和“内部回归”。
