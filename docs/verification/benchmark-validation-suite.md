...
当前已落地三类验证证据，并预留后续工程软件对标接口：

1. 教材解析解：简支梁、悬臂梁、超静定单跨梁、静定桁架等可手算工况。
2. 独立刚度法基线：门式刚架、双跨框架、Pratt 屋架等二维杆系模型。
3. 内部回归基线：用于扩大典型工况、荷载转换和前后端契约覆盖，但不把它包装成外部独立验证。

后续工程软件对标保留接口，未来可引入 Midas / SAP2000 / OpenSees 等外部结果文件，但不得提交第三方专有模型或源码。
...
- 公开验证集当前包含 30 个以上算例，覆盖解析梁、连续梁、框架梁退化验证、典型门式刚架、Pratt / Warren / Howe 桁架与悬挑桁架回归。
...
| 连续梁回归 | 两跨连续梁、三跨连续梁、不等跨连续梁 | 验证三弯矩方程 / 刚度法连续梁链路和内支座边界 |
| 框架梁解析解 | 简支梁跨中集中荷载、简支梁均布荷载、悬臂梁、固端-滚动支座均布荷载 | 验证框架梁柱单元可退化为经典梁问题 |
| 平面桁架解析解 | 三杆静定桁架 | 验证节点平衡、杆件轴力、支座反力和拉压状态 |
| 独立刚度法基线 | 门式刚架、转动弹簧门式刚架、双跨框架、斜杆框架、构件内集中荷载、Pratt / Warren / Howe 屋架 | 验证更接近工程建模的矩阵装配行为 |
| 内部回归基线 | 多跨连续梁、Warren / Howe 桁架、构件自重等效节点荷载、悬挑桁架 | 防止高频工况、荷载转换和结果字段在后续迭代中漂移 |

## 销售材料口径

建议对外表述：

> ArchSight Solver 的公开核心计算链路已接入 CI 回归验证。当前验证集覆盖梁系、二维平面框架、二维平面桁架等典型工况，并对教材解析解、独立刚度法基线和内部回归基线执行自动对标。每次代码变更都会校验最大挠度、构件弯矩、节点位移、杆件轴力和支座反力等专业指标。
...
下一阶段继续补充偏心集中荷载、局部均布荷载、多跨连续梁、K 型支撑、芬克式屋架、侧移刚架、节点转角控制和多荷载组合模型。
