...
> 状态：发布候选（2026-08-06）。本文记录 v1.6.2 之后可靠性与专业验证补丁的仓库内证据。创建 tag、GitHub Release、目标镜像仓库推送和线上更新均未发生，且必须保持为独立决策。
...
| 门禁 | 2026-08-06 结果 |
|---|---|
| 仓库状态 | `main` 相对 v1.6.2 有 28 个提交；候选整理前工作树干净 |
...
| Chromium 交付主链路 | v1.6.1 Reference Host 与 v1.6.2 canonical 工程共 10 项通过 |
...
npm --prefix frontend run test:visual -- release-1-6-2-acceptance.spec.ts release-1-6-1-host-reference.spec.ts --project=chromium --workers=1
git diff --check
```
