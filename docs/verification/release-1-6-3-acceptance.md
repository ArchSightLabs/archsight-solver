...
> 状态：正式发布验收（2026-08-07）。本文记录创建 `v1.6.3` tag 前已经完成的候选证据，以及 Tag 发布工作流必须复跑的正式门禁；其他目标镜像仓库推送和线上更新仍是独立决策。
...
最终发布判断：**GO**。当前代码与候选镜像达到创建 `v1.6.3` tag 和 GitHub Release 的门槛；Tag 工作流全绿是 GitHub Release 与 GHCR 制品发布完成的必要条件，其他目标镜像仓库推送和线上更新继续作为独立的维护者操作。
