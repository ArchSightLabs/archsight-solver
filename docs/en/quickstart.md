...
After the workflow completes, download `archsight_solver-1.9.0-py3-none-any.whl` from the [v1.9.0 GitHub Release](https://github.com/ArchSightLabs/archsight-solver/releases/tag/v1.9.0), then install it into a virtual environment:
...
Download `archsight-solver-v1.9.0.tar.gz` and `SHA256SUMS` from the GitHub Release, verify the archive, and load it:

```bash
sha256sum --check SHA256SUMS --ignore-missing
docker load --input archsight-solver-v1.9.0.tar.gz
...
docker run --rm -p 127.0.0.1:6240:6240 ghcr.io/archsightlabs/archsight-solver:v1.9.0
```
...
Download `archsight-solver-host-client-1.9.0.tgz` from the GitHub Release:

```bash
npm install ./archsight-solver-host-client-1.9.0.tgz
```
