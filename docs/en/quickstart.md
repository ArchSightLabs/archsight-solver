...
After the workflow completes, download `archsight_solver-1.9.1-py3-none-any.whl` from the [v1.9.1 GitHub Release](https://github.com/ArchSightLabs/archsight-solver/releases/tag/v1.9.1), then install it into a virtual environment:

```bash
python -m venv .venv
# Linux/macOS: source .venv/bin/activate
# Windows PowerShell: .\.venv\Scripts\Activate.ps1
python -m pip install ./archsight_solver-1.9.1-py3-none-any.whl
```
...
Download `archsight-solver-v1.9.1.tar.gz` and `SHA256SUMS` from the GitHub Release, verify the archive, and load it:

```bash
sha256sum --check SHA256SUMS --ignore-missing
docker load --input archsight-solver-v1.9.1.tar.gz
...
docker run --rm -p 127.0.0.1:6240:6240 ghcr.io/archsightlabs/archsight-solver:v1.9.1
```
...
Download `archsight-solver-host-client-1.9.1.tgz` from the GitHub Release:

```bash
npm install ./archsight-solver-host-client-1.9.1.tgz
```
