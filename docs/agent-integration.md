...
接入方应优先复用无框架 TypeScript [`SolverHostClient`](host-client.md)，不必自行维护 `postMessage` 会话和保存超时。
...
当前公开核心支持梁系、二维平面桁架和二维平面框架的线弹性静力分析；二维平面框架还可按显式选项执行真实 P-Delta 与整体线性特征屈曲，并返回收敛轨迹、控制来源、模态和残差证据。

不支持：

- 三维杆系和空间框架。
- 动力分析、材料非线性、接触、屈曲后路径以及二维框架能力边界之外的几何非线性；P-Delta 与线性屈曲不等于规范稳定设计。
- 规范设计、施工安全专项和工程签审。
- 多租户认证、云协作和企业权限系统。
