...
`sessionId` 用于关联一次嵌入会话，`nonce` 用于把 launch、变更、保存请求和保存结果绑定到同一轮宿主上下文。前端还支持 `VITE_SOLVER_ALLOWED_HOST_ORIGINS` 作为宿主 origin allowlist。

本仓库提供最小 demo：`examples/host-iframe-demo/index.html`。先启动前端，再用浏览器打开该 HTML，即可模拟 launch、变更消息和保存回传。
