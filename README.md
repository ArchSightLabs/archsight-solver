...
v1.4.0 相对 v1.3.0 的重点不是新增第四类分析对象，而是增强二维平面框架和二维平面桁架的建模效率、荷载表达和验证链路：

- 工作台建模能力增强：支持撤销 / 重做、节点坐标网格吸附、节点 / 构件表格批量编辑，以及复制、镜像、阵列等规则结构构造工具。
- 平面框架荷载体系增强：引入多工况和荷载组合，允许同一结构模型下分别管理基本荷载、温度荷载等不同荷载场景。
- 平面框架温度作用分析：支持二维框架构件的线弹性均匀温度荷载，用于整体升降温引起的构件伸缩、约束反力、杆端内力和节点位移分析。
- 求解器边界补强：补齐支座位移约束、全约束温度荷载反力恢复、材料线膨胀系数传递和结构化工程诊断。
- 验证链路增强：公开 benchmark 已扩展到 61 个算例，并建立验证等级证据链，区分教材解析解、独立刚度法交叉验证、商业软件对标和内部回归样例。

当前温度作用能力仍限定为“线弹性静力 + 构件均匀温差”。截面温度梯度、瞬态热传导、混凝土徐变松弛和桥梁专用温度场分析不作为 v1.4.0 承诺范围；如后续推进，应作为高级温度分析能力单独评估。

v1.4.0 正式发布前仍需完成工作台主路径人工验收，尤其是复制 / 镜像 / 阵列工具的信息层级、撤销 / 重做覆盖、工况切换和温度荷载结果展示。发布判断与剩余事项见 [路线图](docs/roadmap.md)。
...
| [路线图](docs/roadmap.md) | 开源路线、三模块边界、v1.4 候选范围与后续质量路线 | 当前主路线 |
