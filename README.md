...
v1.9.1 将 v1.9.0 之后的嵌入联调修正收口为补丁：首次 Host 握手、主题同步、云端文件菜单、保存状态和参数面板聚焦在不改变求解核心的前提下稳定协作。Solver 继续呈现唯一的 Host Portal 顶栏并保留真实版本、案例、投稿、主题和系统设置；Cloud 通过经协商的文件动作管理工程的新建、打开、保存、另存、版本和分享。账号、订阅和云项目存储仍由 Cloud 宿主负责，不进入开源求解核心。

- 仓库、发行包和 Host Client 的候选版本为 `1.9.1`；Host Protocol 保持 `1.0.0`，portal action 是可选、可回滚的加法能力。
- 本候选不改变既有 GNA/GNIA、线性屈曲或一次分析数值，也不新增结构类型、账号、云项目存储或规范设计能力。
- 完整候选、上线、验证和回滚边界见 [v1.9.1 发布验收](docs/verification/release-1-9-1-acceptance.md)；历次版本变化统一查阅 [CHANGELOG](CHANGELOG.md)。
...
| [v1.9.1 发布验收](docs/verification/release-1-9-1-acceptance.md) | 当前候选的制品、验证、上线与回滚证据 |
