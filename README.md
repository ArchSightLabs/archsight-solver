...
当前状态：**已发布（2026-08-23）**。Tag、GitHub Release、公开离线镜像、校验和、SBOM、安全扫描、容器/Host 与 v1.7 回滚均已有正式证据。GHCR 是发布工作流使用的镜像副本，可能需要 GitHub Packages 授权；官方线上部署使用阿里云容器镜像服务，镜像推送与服务器更新保持为独立运维决策。
