...
v1.6.0 的发布方向是“中性宿主接入 + 本地项目契约增强”，不是把开源核心做成商业平台。本版围绕外部嵌入和项目证据链补齐以下能力：

- Host iframe 消息协议、origin allowlist、保存请求 / 保存结果事件和本地 demo，便于外部系统嵌入个人工具。
...
完整任务边界见 [路线图](docs/roadmap.md)，发布复核见 [v1.6.0 发布验收清单](docs/verification/release-1-6-acceptance.md)。
...
| [v1.6.0 发布验收清单](docs/verification/release-1-6-acceptance.md) | 中性宿主、项目契约、版本、容器制品和回滚门禁 | v1.6.0 发布基线 |
